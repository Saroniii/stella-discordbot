...
class LevelStaticTableSection(SelectableSectionSpec):
    name = "level-static-table"
    schema_version = 1

    def default_payload(self) -> dict[str, Any]:
        return LevelStaticTableConfigV1().model_dump(mode="json")

    def validate_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            return LevelStaticTableConfigV1.model_validate(payload).model_dump(mode="json")
    ...
    def render_show(self, now_config: dict[str, Any], deploy_config: dict[str, Any] | None) -> str:
        def build(source: dict[str, Any] | None) -> list[str]:
            root = CliNode(kind="enter", text="enter level-static-table")
